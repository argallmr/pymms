...
def fsm_spectra(sc, start_date, end_date):
    ...
    duration = None
    if duration is None:
        nperseg_fgm = len(fgm_data['time'])
        nperseg_scm = len(scm_data['time'])
        nperseg_fsm = len(fsm_data['time'])
    else:
    ...
    f_fgm, pxx_fgm = welch(fgm_data['B_GSE'], axis=0,
                           fs=fs_fgm, window='hann', nperseg=nperseg_fgm,
                           detrend='linear', return_onesided=True,
                           scaling='density')
    f_scm, pxx_scm = welch(scm_data['B_GSE'], axis=0,
                           fs=fs_scm, window='hann', nperseg=nperseg_scm,
                           detrend='linear', return_onesided=True,
                           scaling='density')
    f_fsm, pxx_fsm = welch(fsm_data['B_GSE'], axis=0,
                           fs=fs_fsm, window='hann', nperseg=nperseg_fsm,
                           detrend='linear', return_onesided=True,
                           scaling='density')
    ...
    fig, axes = plt.subplots(nrows=3, ncols=1, squeeze=False,
                             figsize=(4.5, 6.5))
    plt.subplots_adjust(left=0.2, right=0.95, top=0.9, bottom=0.08,
                        hspace=0.1, wspace=0.4)
    
    # Create a title using the timestamp
    if start_date.date() == end_date.date():
        ...
        tstamp = ' '.join((start_date.strftime('%Y-%m-%d'),
                           ...
                           end_date.strftime('%H:%M:%S')))
    title = 'FGM, SCM, & FSM Spectra Comparison\n{0}'.format(tstamp)

    # Bx PSD
    ax = axes[0,0]
    ax.plot(f_fsm, np.sqrt(pxx_fsm[:,0]), label='FSM', linewidth=3)
    ax.plot(f_scm, np.sqrt(pxx_scm[:,0]), label='SCM', linewidth=1)
    ax.plot(f_fgm, np.sqrt(pxx_fgm[:,0]), label='FGM', linewidth=1)
    ax.set_title(title)
    ax.set_xscale('log')
    ax.set_xlabel('')
    ax.set_xticklabels([])
    ...
    # By PSD
    ax = axes[1,0]
    ax.plot(f_fsm, np.sqrt(pxx_fsm[:,1]), label='FSM', linewidth=3)
    ax.plot(f_scm, np.sqrt(pxx_scm[:,1]), label='SCM', linewidth=1)
    ax.plot(f_fgm, np.sqrt(pxx_fgm[:,1]), label='FGM', linewidth=1)
    ax.set_xscale('log')
    ax.set_xlabel('')
    ax.set_xticklabels([])
    ax.set_yscale('log')
    ...
    # Bz PSD
    ax = axes[2,0]
    ax.plot(f_fsm, np.sqrt(pxx_fsm[:,2]), label='FSM', linewidth=3)
    ax.plot(f_scm, np.sqrt(pxx_scm[:,2]), label='SCM', linewidth=1)
    ax.plot(f_fgm, np.sqrt(pxx_fgm[:,2]), label='FGM', linewidth=1)
    ax.set_xscale('log')
    ax.set_xlabel('f (Hz)')
    ax.set_yscale('log')
